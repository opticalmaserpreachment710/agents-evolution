...
## Context Retention — защита от потери инструкций
...
- **Pre-Action Check** — перед деплоем, запуском сервера, коммитом, изменением окружения, взаимодействием с внешними сервисами. Агент проверяет конфиги, `.env`, `.gitignore`, отсутствие секретов в staged
- **Boundaries** — проверка границ: не сканировать `node_modules/`, `vendor/`, `.git/`, `dist/`; не коммить секреты; использовать `reduce` вместо `Math.max(...arr)`

### Точечное чтение при жалобах
**По запросу пользователя** — если вы сообщаете о проблеме, агент не перечитывает весь `AGENTS.md`, а сразу обращается к нужной секции:
- Жалоба на секреты → `Pre-Action Check` + `.gitignore`
- Жалоба на производительность → `Производительность` + `accelint-ts-performance`
- Жалоба на тесты → `Тестирование` + `accelint-ts-testing`

Это экономит токены — не тратится время на полную перечитку.

### Если агент теряет инструкции
...
### Скиллы усиливают контекст
Помимо `AGENTS.md`, контекст дополняется скиллами:
- **Профессиональные скиллы** (`.agents/skills/`) — имеют свои триггеры, автоматически активируются по ключевым словам
- **Пользовательские скиллы** (`.myskills/skills/`) — ваши собственные паттерны, накопленные через сессии
- **Проектные скиллы** (`.skills/`) — копии используемых скиллов, доступные в контексте проекта

Каждый скилл содержит `SKILL.md` с описанием, триггерами активации и правилами. Агент автоматически сканирует доступные скиллы и применяет релевантные.
